...
Já documentado no `README.md` do repositório: copiar uma pasta existente em `cursos/` ou `extensao/` (o `index.html` e o `index-matrix.html` dela), ajustar o conteúdo, e adicionar o card correspondente nas duas homes (`index.html` e `index-matrix.html`).
